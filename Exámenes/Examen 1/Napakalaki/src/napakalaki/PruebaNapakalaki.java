...
public class PruebaNapakalaki {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i<totalMonstruos; i++){
            ...
            if(monsterAux.getBC().getLevels()>0 && 
                    monsterAux.getBC().getNVisibleTreasures()==0 &&
                    monsterAux.getBC().getNHiddenTreasures()==0 &&                    
                    monsterAux.getBC().getTVisible().isEmpty() &&
                    monsterAux.getBC().getTHidden().isEmpty()){
            ...
            if(monsterAux.getBC().getNHiddenTreasures()>0 || 
                    monsterAux.getBC().getNVisibleTreasures()>0){
        ...
        // EXAMEN-inicio
        AlterEgo alterEgo = new AlterEgo(18, listMonster10);
        
        // Se lo asigno al primero de la lista
        monstruos.get(0).setMiAlterEgo(alterEgo);
        
        System.out.println("\nLista completa de monstruos con su alter ego mayor"
                + " que 10: ");
        for (Monster monstruo : monstruos) {
            if (monstruo.getMiAlterEgo().getNivelDeBondad() > 10) {
